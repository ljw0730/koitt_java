...
//		input_dumy();
		...
		kor = input_score(name, 3);
		eng = input_score(name, 4);
		math = input_score(name, 5);
	...
	public int input_score(String name, int subject_num) {
		int score = 0;
		while(true) {
			System.out.print(name + " 학생의 " + title[subject_num] + "점수를 입력해 주세요. : ");
			if(!scan.hasNextInt()) {
			...
			if(score < 0 || score > 100) {
				System.out.println("잘못 입력하셨습니다. 점수는 0~100 사이의 값이어어 합니다.");
				score = 0;
				continue;
			}
			return score;
